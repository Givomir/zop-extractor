...
- начин_на_възлагане: видът процедура, ТОЧНО както е посочен в документа. Възможни видове по ЗОП: "Публично състезание", "Открита процедура", "Ограничена процедура", "Състезателна процедура с договаряне", "Състезателен диалог", "Пряко договаряне", "Договаряне без предварително обявяване". Извлечи точния вид от текста — НЕ предполагай "открита процедура" по подразбиране.
- кратко_описание: описанието на услугата, НЕ идентификатори или номера
- критерии_за_подбор: ИЗИСКВАНИЯ КЪМ УЧАСТНИЦИТЕ за допускане (опит, оборот, персонал, сертификати, технически способности) като списък от текстове. ТОВА НЕ СА оценъчните критерии.
- изисквания_за_сертификация: ISO 9001 или друг сертификат
- критерии_за_възлагане: ОЦЕНЪЧНИТЕ критерии за класиране на офертите, като СПИСЪК от текстове. Всеки елемент описва един критерий с неговата тежест ТОЧНО както е в документа (напр. "Предлагана цена – 90 точки", "Оценка на качеството – 10 точки"). НЕ изчислявай и НЕ измисляй проценти — копирай теглата дословно. ТОВА НЕ СА изискванията за подбор.
- краен_срок_за_подаване: КРАЙНАТА ДАТА за подаване/получаване на оферти (конкретна дата, напр. "06-юли-2026"). ВНИМАНИЕ: това НЕ е "срокът на валидност на офертата" (който е в месеци) и НЕ е срокът за изпълнение на договора.
- дата_на_публикуване: датата на публикуване или изпращане на обявлението (конкретна дата като "08-юни-2026"). ВНИМАНИЕ: НЕ използвай регистрационни/идентификационни номера (напр. "20260528-0277-0016") като дата."""
...
# затова гласуването избира правилното заглавие вместо случаен дълъг текст.
CONSENSUS_FIELDS = {
    "възложител", "начин_на_възлагане", "прогнозна_стойност",
    "краен_срок_за_подаване", "дата_на_публикуване",
    "предмет", "кратко_описание",
}
...
def canonicalize_procedure(val):
    ...
    if not isinstance(val, str):
        return val
    low = val.strip().lower()
    for key, canonical in PROCEDURE_TYPES.items():
        if key in low:
            return canonical
    return val
